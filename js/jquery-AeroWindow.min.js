 /*
 *
 * Windows 7 UI & Windows Aero for Websites - jQuery Plugin (v2.5)
 * Copyright 2010, Christian Goldbach
 * Dual licensed under the MIT or GPL Version 2 licenses.
 * Can be used in GPL applications / NOT for Commercial use / Licensing information: info@soyos.net
 * 
 * Project Website:
...
(function($) {
	var AeroWindowPlugin = function(element, options) 
	{
		...
		var elem = $(element);
		var obj = this;
		var settings = $.extend({WindowTitle: 'Windows for jQuery',WindowDesktopIconFile: 'images/icons/default.png',WindowStatus: 'window',WindowPositionTop: 200,WindowPositionLeft: 100,WindowOuterWidth: 300,WindowOuterHeight: 300,WindowMinWidth: 100,WindowMinHeight: 100,WindowDesktopIcon: true,WindowResizable: true,WindowMaximize: true,WindowMinimize: true,WindowClosable: true,LoadiFrameContentLater: true,WindowDraggable: true,WindowAnimationSpeed: 500,WindowAnimation: 'easeOutCubic',WindowElementID: this.id,WindowTransparentAnimationSpeed: 300,WindowBorderWidth: 22,WindowBorderHeight: 62,WindowBorderMouseCursorSpacing: 25,TaskbarHeight: 40}, options || {});
		...
		if (settings.WindowPositionLeft == 'center') {
			settings.WindowPositionLeft = ($(window).width() / 2) - settings.WindowOuterWidth / 2;
                ...
                var imprimir= '<a href="#" class="win-printer-btn" onclick="imprimir_frame(\''+element.id+'-iframe\')" ></a>';
		...
		$(elem).html('<div class="AeroWindow">' + '  <table cellpadding="0" cellspacing="0" border="0">' + '    <tr>' + '      <td class="table-tl"></td>' + '      <td class="table-tm"></td>' + '      <td class="table-tr"></td>' + '    </tr>' + '    <tr>' + '      <td class="table-lm"></td>' + '      <td class="table-mm" align="right">' + '        <div class="title"><nobr><img src="' + settings.WindowDesktopIconFile + '"><span>' + settings.WindowTitle + '</span></nobr></div>' + '        <div class="buttons">' + 
		WinBtnLeftedge + 
                imprimir +
		WinMinBtn + 
		WinMaxBtn + 
		WinRegBtn + 
		WinCloseBtn + 
		WinBtnRightedge + '        </div>' + '        <div class="table-mm-container" align="left">' + '          <div class="table-mm-content" style="width: ' + (settings.WindowOuterWidth - settings.WindowBorderWidth) + 'px; height: ' + (settings.WindowOuterHeight - settings.WindowBorderHeight) + 'px;">' + 
		SourceContent + '          </div>' + '        </div>' + '      </td>' + '      <td class="table-rm"></td>' + '    </tr>' + '    <tr>' + '      <td class="table-bl"></td>' + '      <td class="table-bm"></td>' + '      <td class="table-br"></td>' + '    </tr>' + '  </table>' + '</div>');
		...
		var BTNClose = $(elem).find(".win-close-btn");
                var BTNIMP = $(elem).find(".win-printer-btn");
		...
		WindowContainer.draggable({distance: 3,cancel: '.table-mm-content',start: function() {
				...
				WindowContainer.draggable({cursorAt: null});
			}});
		if (settings.WindowResizable) {
			WindowContainer.resizable({minHeight: settings.WindowMinHeight,minWidth: settings.WindowMinWidth,alsoResize: WindowContent,handles: 'n, e, s, w, ne, se, sw, nw',start: function() {
		...
		this.TaskbarHandler = function() {
			...
			$("#Taskbar" + this.id).live("click", function() {
		...
		this.DesktopIconHandler = function() {
			$('#DesktopIcons .DesktopIconContainer').live("mouseover mouseout", function(event) {
				if (event.type == 'mouseover') 
				{
			...
			$('#DesktopIcon' + this.id).live("click", function() {
			...
			$('#DesktopIcon' + this.id).dblclick(function(e) {
				if (settings.WindowStatus == 'minimized') {
					obj.ResizeWindow('restore');
				} else if (settings.WindowStatus == 'window') {
					obj.FocusWindow();
				} else if (settings.WindowStatus == 'closed') {
					if (settings.WindowURL && $("#" + obj.id + "-iframe") !== undefined) 
					{
						$("#" + obj.id).find('.table-mm-content').addClass('loading');
						$("#" + obj.id + "-iframe").css("visibility", "hidden");
						window.setTimeout(function() {
							$("#" + obj.id + "-iframe").css("display", "none").attr("src", settings.WindowURL);
							$("#" + obj.id + "-iframe").load(function() {
								$("#" + obj.id).find('.table-mm-content').removeClass('loading');
					...
					switch (settings.WindowStatusBefore) {
						case 'window':
							...
							EndWindowOuterWidth = 200;
							EndWindowOuterHeight = 100;
							settings.WindowTop = e.pageY - (StartWindowOuterHeight / 2);
							settings.WindowLeft = e.pageX - (StartWindowOuterWidth / 2);
							settings.WindowOuterWidth = StartWindowOuterWidth;
							settings.WindowOuterHeight = StartWindowOuterHeight;
							obj.ResizeWindow('changeSizeFast');
							obj.ResizeWindow('restore');
							settings.WindowStatus = 'onChange';
							settings.WindowOuterWidth = EndWindowOuterWidth;
							settings.WindowOuterHeight = EndWindowOuterHeight;
							settings.WindowTop = ($(window).height() / 2) - (settings.WindowOuterHeight / 2);
							settings.WindowLeft = ($(window).width() / 2) - (settings.WindowOuterWidth / 2);
							obj.ResizeWindow('changeSize');
							settings.WindowStatus = 'onChange';
							EndWindowOuterWidth = 800;
							EndWindowOuterHeight = 400;
							MinSpacing = 20;
							if ((($(window).width() - MinSpacing) < EndWindowOuterWidth) || (($(window).height() - MinSpacing) < EndWindowOuterHeight)) {
		...
		this.ResizeWindow = function(NewSize, FocusWindow, WindowStatusBefore) {
			...
			switch (NewSize) {
				case 'transparent-on':
					WindowContentContainer.animate({opacity: 0.0}, {queue: false,duration: settings.WindowTransparentAnimationSpeed});
					break;
				case 'transparent-off':
					WindowContentContainer.animate({opacity: 1.0}, {queue: false,duration: settings.WindowTransparentAnimationSpeed});
					break;
				case 'minimize':
					...
					if ($.browser.msie) {
						...
						Window.animate({opacity: 'hide'}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					}
					WindowContainer.animate({width: settings.WindowMinWidth,height: settings.WindowMinHeight,top: -100 + $('#Taskbar' + this.id).offset().top,left: $('#Taskbar' + this.id).offset().left}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation,complete: function() {
							WindowContainer.css('display', 'none');
						}});
					WindowContent.animate({width: settings.WindowMinWidth - settings.WindowBorderWidth,height: settings.WindowMinHeight - settings.WindowBorderHeight}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					WindowContainer.draggable({cursorAt: {cursor: "crosshair",top: settings.WindowBorderMouseCursorSpacing,left: (settings.WindowOuterWidth / 2)}});
				...
				case 'maximize':
					...
					if ($.browser.msie) {
						...
						Window.animate({opacity: 'show'}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					}
					WindowContainer.draggable({disabled: true});
					WindowContainer.animate({width: $(window).width(),height: (ShowTaskbar == true ? $(window).height() - settings.TaskbarHeight : $(window).height()),top: $(window).scrollTop(),left: $(window).scrollLeft()}, {duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					WindowContent.animate({width: $(window).width() - settings.WindowBorderWidth,height: (ShowTaskbar == true ? $(window).height() - settings.WindowBorderHeight - settings.TaskbarHeight : $(window).height() - settings.WindowBorderHeight)}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation,complete: function() {
				...
				case 'restore':
					...
					Window.animate({opacity: 'show'}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					if ($.browser.msie) {
						WindowContainer.animate({width: settings.WindowOuterWidth,height: settings.WindowOuterHeight,top: settings.WindowTop + $(window).scrollTop(),left: settings.WindowLeft + $(window).scrollLeft()}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					} else {
						WindowContainer.animate({opacity: 1,width: settings.WindowOuterWidth,height: settings.WindowOuterHeight,top: settings.WindowTop,left: settings.WindowLeft}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					}
					WindowContent.animate({width: settings.WindowOuterWidth - settings.WindowBorderWidth,height: settings.WindowOuterHeight - settings.WindowBorderHeight}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					WindowContainer.draggable({cursorAt: null});
				...
				case 'changeSize':
					...
					WindowContainer.animate({width: settings.WindowOuterWidth,height: settings.WindowOuterHeight,top: settings.WindowTop + $(window).scrollTop(),left: settings.WindowLeft + $(window).scrollLeft()}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					WindowContent.animate({width: settings.WindowOuterWidth - settings.WindowBorderWidth,height: settings.WindowOuterHeight - settings.WindowBorderHeight}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation});
					WindowContent.find('iframe').animate({opacity: 'show'}, {queue: true,duration: 2000});
					break;
				case 'changeSizeFast':
					WindowContainer.css({width: settings.WindowOuterWidth,height: settings.WindowOuterHeight,top: settings.WindowTop + $(window).scrollTop(),left: settings.WindowLeft + $(window).scrollLeft()}, {});
					WindowContent.css({width: settings.WindowOuterWidth - settings.WindowBorderWidth,height: settings.WindowOuterHeight - settings.WindowBorderHeight}, {});
					break;
				case 'restoreToMouse':
					BTNMin.css('display', 'block');
					BTNRes.css('display', 'none');
					BTNMax.css('display', 'block');
					settings.WindowStatus = 'window';
					WindowContainer.css({width: settings.WindowOuterWidth,height: settings.WindowOuterHeight});
					WindowContent.css({width: settings.WindowOuterWidth - settings.WindowBorderWidth,height: settings.WindowOuterHeight - settings.WindowBorderHeight});
					WindowContainer.resizable('enable');
					break;
				case 'close':
					...
					if ($.browser.msie) {
						...
						WindowContainer.animate({opacity: 0}, {queue: true,duration: settings.WindowAnimationSpeed,easing: settings.WindowAnimation,complete: function() {
								WindowContainer.css('display', 'none');
							}});
					}
		...
		function init(id) {
			...
			if (settings.WindowDesktopIcon == true) {
				...
				$('#DesktopIcons').append('<div class="DesktopIconContainer" id="DesktopIcon' + id + '">' + '<table cellpadding="0" cellspacing="0" border="0" title="' + settings.WindowTitle + '">' + '  <tr>' + '    <td class="bg-top"></td>' + '  </tr>' + '  <tr>' + '    <td class="bg-middle"><img src="' + settings.WindowDesktopIconFile + '" width="48" height="48"></td>' + '  </tr>' + '  <tr>' + '    <td class="bg-bg"><p>' + DesktopIconCaption + '</p></td>' + '  </tr>' + '  <tr>' + '    <td class="bg-bottom"></td>' + '  </tr>' + '</table>' + '</div>');
				$('#DesktopIcon' + id).draggable({helper: "original",start: function() {
						$('#DesktopIcons .DesktopIconContainer').removeClass('mouseoverclicked');
						$('#DesktopIcons .DesktopIconContainer').removeClass('mouseoverclickedmouseout');
						$(this).addClass('mouseoverclicked');
					}});
	...
	$.fn.AeroWindowLink = function(options) {
		var options = $.extend({WindowURL: null,WindowTitle: 'Windows for jQuery',WindowDesktopIconFile: ImagePath + '/icons/default.png',WindowStatus: 'window',WindowPositionTop: 'center',WindowPositionLeft: 'center',WindowOuterWidth: 800,WindowOuterHeight: 500,WindowMinWidth: 100,WindowMinHeight: 100,WindowDesktopIcon: true,WindowResizable: true,WindowMaximize: true,WindowMinimize: true,WindowClosable: true,WindowDraggable: true,WindowAnimationSpeed: 500,WindowAnimation: 'easeOutCubic',WindowTransparentAnimationSpeed: 300}, options || {});
		return this.each(function() {
			var $this = $(this);
			var img = $("img", $this).attr("src") ? $("img", $this).attr("src") : ImagePath + '/icons/default.png';
			var id = "window-" + rnd();
			$('body').append('<div id="' + id + '" style="display: none;">' + '<iframe src="" width="100%" height="100%" style="display:none; border: 0px;" frameborder="0" name="' + id + '-iframe" id="' + id + '-iframe"></iframe>' + '</div>');
			options.WindowURL = $this.attr("href");
			options.WindowTitle = $this.text();
			options.WindowDesktopIconFile = img;
			options.WindowStatus = "closed";
...
$(document).ready(function() {
	function minallDelay(Counter) {
		...
		if (Counter > 0) {
			Counter = Counter - 1;
			if (Windows[Counter].get('WindowStatus') != 'minimized' && Windows[Counter].get('WindowStatus') != 'closed') {
	...
	function SortGrid() {
		...
		jQuery.each($('body').data('AeroWindows'), function() {
			if (this.get('WindowStatus') != 'minimized' && this.get('WindowStatus') != 'closed') {
				...
				WindowCount += 1;
				$('#' + this.id).css({top: AeroWindowPositionTop,left: AeroWindowPositionLeft,width: AeroWindowWidth,height: AeroWindowHeight - RowSpaceForTaskbar});
				$('#' + this.id).find(".table-mm-content").css({width: AeroWindowWidth - this.get('WindowBorderWidth'),height: AeroWindowHeight - RowSpaceForTaskbar - this.get('WindowBorderHeight')});
			}
		});
	}
	;
	function SortOverlapping() {
		...
		jQuery.each($('body').data('AeroWindows').reverse(), function() {
			if (this.get('WindowStatus') != 'minimized' && this.get('WindowStatus') != 'closed') {
				...
				;
				$('#' + this.id).css({top: AeroWindowPositionTop,left: AeroWindowPositionLeft,width: AeroWindowWidth,height: AeroWindowHeight});
				$('#' + this.id).find(".table-mm-content").css({width: AeroWindowWidth - this.get('WindowBorderWidth'),height: AeroWindowHeight - this.get('WindowBorderHeight')});
				counter2 += 1;
			}
		});
	}
	...
	function resallDelay(Counter) {
		...
		if (Counter > 0) {
			Counter = Counter - 1;
			if (Windows[Counter].get('WindowStatus') != 'window' && Windows[Counter].get('WindowStatus') != 'closed') {
				AnimationSpeed = Windows[Counter].get('WindowAnimationSpeed');
	...
	function GenerateNewWindow(NewWindowURL, WindowType, ShowDesktopIcon) {
		...
		if (WindowType == 'image') {
			$('body').append('<div id="' + Randomnumber + '">' + '  <image src="' + NewWindowURL + '" style="border: 0px;">' + '</div>');
			if ((($(window).width() - 32 - 50) < $('#' + Randomnumber).find('img').width()) || (($(window).height() - 72 - 50) < $('#' + Randomnumber).find('img').height())) {
			...
			$('#' + Randomnumber).find('img').css({width: '100%',height: '100%'});
		} else {
			$('body').append('<div id="' + Randomnumber + '">' + '  <iframe src="' + NewWindowURL + '" width="100%" height="100%" style="border: 0px;" frameborder="0"></iframe>' + '</div>');
		...
		$('#' + Randomnumber).AeroWindow({WindowDesktopIconFile: image,WindowTitle: NewWindowURL,WindowPositionTop: 'center',WindowPositionLeft: 'center',WindowStatus: 'window',WindowOuterWidth: WindowOuterWidth,WindowOuterHeight: WindowOuterHeight,WindowDesktopIcon: ShowDesktopIcon});
		$("body #tmpIcon").remove();
	}
	function GenerateNewWindowLightbox(NewWindowURL, WindowTitle, ShowDesktopIcon, e) {
		...
		if (WindowType == 'Image') {
			...
			$(img).load(function() {
				$(this).hide();
				$('#' + Randomnumber).find('.table-mm-content').html(this);
				$(this).fadeIn();
				MinSpacing = 20;
				if ((($(window).width() - 32 - MinSpacing) < $(this).width()) || (($(window).height() - 72 - MinSpacing) < $(this).height())) {
				...
				$('#' + Randomnumber).find('.table-mm-content').find('img').css({width: '100%',height: '100%'});
				$('#' + Randomnumber).find('.table-mm-content').removeClass('loading');
				AnimateWindows();
			}).error(function() {
				$('#' + Randomnumber).find('.table-mm-content').removeClass('loading');
				$('#' + Randomnumber).find('.table-mm-content').html('<p>This Picture was not found...</p>');
				AnimateWindows();
		...
		function ShowWindow() {
			if (!$.browser.msie) {
				$('#' + Randomnumber).css({opacity: 0.5});
			}
			$('#' + Randomnumber).AeroWindow({WindowDesktopIconFile: image,WindowTitle: WindowTitle,WindowPositionTop: e.pageY - (StartWindowOuterHeight / 2),WindowPositionLeft: e.pageX - (StartWindowOuterWidth / 2),WindowStatus: 'window',WindowOuterWidth: StartWindowOuterWidth,WindowOuterHeight: StartWindowOuterHeight,WindowDesktopIcon: false,WindowMinimize: true,WindowDesktopIcon: ShowDesktopIcon});
			...
			if (WindowType == 'Link') {
				window.setTimeout(function() {
					...
					var ContentsTimer = setTimeout(function() {
						$('#' + Randomnumber).find('.table-mm-content').removeClass('loading');
						$('#' + Randomnumber).find('.table-mm-content').html('<p>This website was not found... <br>Your URL must be start with <i>HTTP://www...</i></p>');
					}, 5000);
	...
	if ($('#Taskbar').length == 0) {
		$('body').append('<div id="Taskbar" style="display: none"></div>');
	...
	$.get(AjaxPath + '/StartmenuContent.html', function(data) {
		$('#Start-Menu').html(data);
		$('#Start-Menu').load('ajax/StartmenuContent.html', function() {
	...
	$('#Start-Menu-Button').live("mouseover mouseout", function(event) {
		if (event.type == 'mouseover')
			$(this).addClass('hover');
		else
			$(this).removeClass('hover');
	});
	$('#Start-Menu-Button').live("click", function() {
	...
	$('#Start-Menu #NavItems li').live("mouseover mouseout", function(event) {
		if (event.type == 'mouseover')
			$(this).addClass('hover');
		else
			$(this).removeClass('hover');
	...
	$('html').click(function(event) {
		...
		if ((event.target.id == 'Start-Menu') || ($(event.target).parents('#Start-Menu-Button').length || event.target.id == 'Start-Menu-Button') || ($(event.target).parents('#SearchBox').length || event.target.id == 'SearchBox')) {
			outClicked_startmenu = false;
		}
		if ($(event.target).parents('#DesktopIcons').length) {
			outClicked_desktopicon = false;
		}
		if ($(event.target).parents('.AeroWindowContainer').length || $(event.target).hasClass('AeroWindowContainer') || $(event.target).parents('.Taskbar-Item').length || $(event.target).hasClass('Taskbar-Item')) {
